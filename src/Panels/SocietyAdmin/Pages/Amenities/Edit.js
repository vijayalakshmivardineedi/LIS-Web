...
const Edit = () => {
  ...
  const amenity = useSelector(state => state.amenitieslist.amenities);
  const status = useSelector((state) => state.amenitieslist.status);
  const error = useSelector((state) => state.amenitieslist.error);
  const successMessage = useSelector((state) => state.amenitieslist.successMessage);
  ...
  useEffect(() => {
    if (amenity) {
      ...
      const imagePreviews = amenity.image ? [`${ImagebaseURL}${amenity.image}`] : [];
